...
public static HashMap<Integer, Node> idToNode = new HashMap<Integer, Node>();
...
* @config
*/
public static final String PAR_MAXSIZE = "network.initialCapacity";
...
* @config
*/
public static final String PAR_SIZE = "network.size";
...
//	Communicator.log("len is " + len);
//	System.exit(1);
	
	int maxlen = Configuration.getInt(PAR_MAXSIZE,len);
	
	//Communicator.log(maxlen + " vs " + len);
...
public static void add( Node n ) {
	...
	if(n.getProtocol(KademliaProtocol.kademliaid) instanceof IPeer) {
		IPeer peer = (IPeer)n.getProtocol(KademliaProtocol.kademliaid);
		idToNode.put(peer.getId(), n);
	}
}
...
public static void addIPeer(IPeer peer) {
	idToNode.put(peer.getId(), peer.toNode());
}

public static void clearIdToNode() {
	idToNode = null;
}
