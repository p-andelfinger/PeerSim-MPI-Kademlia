...
/**
 * the current time, divided by 1000 and parsed into an Integer by cutting
 * the first 33 bits, thus a 31 bit - i.e. positive - Integer remains. This
 * is used by the {@link KademliaProtocol} for performance reasons, where
 * times are interpreted as [ms]. Thus, in Kademlia simulations, this field
 * represents the current time in [s], truncated to fit an Integer. @see
 * #getKademliaSecTime()
 */
private static int kadSecTime = 0;
...
/**
 * Returns the current time, divided by 1000 and parsed into an Integer by
 * cutting the first 33 bits, thus a 31 bit - i.e. positive - Integer
 * remains. This method is used by the {@link KademliaProtocol} for
 * performance reasons, where times are interpreted as [ms]. Thus, in
 * Kademlia simulations, this method returns the current time in [s],
 * truncated to fit an Integer.
 * 
 * @return the current time divided by 1000 and parsed into an Integer by
 *         cutting the first 33 bits.
 */
public static int getKademliaSecTime() {
	return kadSecTime;
}
//-----------------------------------------------------------------
...
{
	time = t;
	// see getKademliaSecTime()
	kadSecTime = (int) ((t / 1000) & Integer.MAX_VALUE);
}
